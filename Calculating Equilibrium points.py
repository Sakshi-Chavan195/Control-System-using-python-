...
def find_equilibrium_points():
    ...
    # Step 4: Set x1_dot and x2_dot equal to zero and solve for equilibrium points
    equi_points = solve([Eq(x1_dot_subs, 0), Eq(x2_dot_subs, 0)], (x1, x2))
...
def find_eigen_values(A_matrices):
    ...
    for A in A_matrices:
        ...
        # Stability check
        stable = all(val.as_real_imag()[0] < 0 for val in eigvals.keys())
...
def compute_lqr_gain(jacobians_A, jacobians_B):
    ...
    # Select the A and B matrices corresponding to an unstable point
    for A, B in zip(jacobians_A, jacobians_B):
        A_num = np.array(A).astype(np.float64)
        B_num = np.array(B).astype(np.float64)

        # Compute LQR gain
        K, _, _ = lqr(A_num, B_num, Q, R)
